...
async fn test_sse_list() {
    ...
    let url = format!("{}/api/clipboards", server.server_address().unwrap().to_string().trim_end_matches('/'));
...
async fn test_sse_list_updates() {
    ...
    let form = make_clipboard_form("clip1", 300, None, vec![], false).await;
    server
        .post("/api/clipboards")
        .multipart(form)
        .await
...
async fn test_sse_clipboard_events() {
    ...
    // Create a clipboard
    let form = make_clipboard_form("clip", 300, Some("initial"), vec![], false).await;
    server
        .post("/api/clipboards")
        .multipart(form)
        .await
    ...
    let url = format!("{}/api/clipboards/clip/events", server.server_address().unwrap().to_string().trim_end_matches('/'));
